...
const NUMBER_OF_TIMES_TO_REPEAT = 10000;
 ...
 */
function insertKeys(hashTable, storage, numberofkeys) {
...
function analyseHashTable(hashTable) {
  for (let i = 0; i < NUMBER_OF_TIMES_TO_REPEAT; i++) {
    const insertedKeys = [];
    insertKeys(hashTable, insertedKeys, 1);
...
function startOneTimeAnalysis() {
  ...
  const choosenMaxLoadFactor = 0.85;
